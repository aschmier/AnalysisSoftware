...
//         if ((fEnergyFlag.CompareTo("pPb_5.023TeV") == 0 || fEnergyFlag.CompareTo("pPb_5.023TeVCent") == 0 ) && (mode == 2 || mode == 3 || mode == 4 ) ) fEventCutSelectionRead.Replace(3,1,"0");
    ...
    if(HistosGammaConversion == NULL){
        ...
        //******************************************************************************************************
        if ( (mode == 2 || mode == 4 || mode == 12 || mode == 13  || mode==14) && optionMC.CompareTo("kTRUE") == 0 ){
...
//             fEventCutSelectionRead          = fEventCutSelection;
            ...
            while (HistosGammaConversion == NULL && i < 20){
                ...
                fClusterCutSelectionRead= fClusterCutSelection;
                if (mode==2 || mode==13 || mode==102 || mode==113 || mode==14)
                    fCutSelectionRead       = Form("%s_%s_%s_%s",fEventCutSelectionRead.Data(), fGammaCutSelection.Data(), fClusterCutSelection.Data(), fMesonCutSelection.Data());
                else
                    fCutSelectionRead       = Form("%s_%s_%s",fEventCutSelectionRead.Data(), fClusterCutSelection.Data(), fMesonCutSelection.Data());
   ...
   if(fDoJetAnalysis){
    ...
    if(fCrysFitting==0){
        ...
        if(optionOtherResBckAsStd == -1){
            ...
            FitSubtractedInvMassInPtBinsWOLinear(fFittingHistMidPtSignalSub, fMesonIntDeltaRange,200,kTRUE);
        }
...
//         }
        ...
        if(fCrysFitting==0){
            ...
            if( optionOtherResBckAsStd == -2 ){
              FitSubtractedInvMassInPtBinsWOLinear(fHistoMappingSignalInvMassPtBin[iPt], fMesonIntDeltaRange,iPt,kFALSE);
    ...
    if (fCrysFitting == 0){
        ...
        if (iBckSwitch == 5){
            ...
            PlotWithManyFitSigToBckRatioInPtBins(   fHistoMappingRatioSBInvMassPtBin, fFitPHOSPolSigToBckFitPtBin, fFitPHOSAllOtherSigToBckFits, iNumberOfOtherSigToBckRatioFits, labelsOtherFitsRatio, nameMesonSub,
                                                    nameCanvasSub, namePadSub, fMesonMassPlotRange, fDate, fPrefix, fRow, fColumn, fStartPtBin, fNBinsPt, fBinsPt,
                                                    fTextMeasurement, fIsMC,fDecayChannel, fDetectionProcess, fCollisionSystem, "MC validated", kTRUE, Form("%s SigToBG Fit", labelUsedFitsRatio.Data()), optionBackgroundStr);
            ...
            PlotWithManyFitSigToBckRatioInPtBins(   fHistoMappingRatioSBInvMassPtBin, fFitPHOSPolSigToBckFitPtBin_withGaus, fFitPHOSAllOtherSigToBckFits_withGaus, iNumberOfOtherSigToBckRatioFits, labelsOtherFitsRatio_withGaus, nameMesonSub,
                                                    nameCanvasSub, namePadSub, fMesonMassPlotRange, fDate, fPrefix, fRow, fColumn, fStartPtBin, fNBinsPt, fBinsPt,
                                                    fTextMeasurement, fIsMC,fDecayChannel, fDetectionProcess, fCollisionSystem, "MC validated", kTRUE, Form("%s withGaus SigToBG Fit", labelUsedFitsRatio.Data()), optionBackgroundStr);
        ...
        PlotWithBGFitSubtractedInvMassInPtBins(
            fHistoMappingSignalInvMassLeftPtBin, fHistoMappingRemainingBGInvMassLeftPtBin, fHistoMappingSignalRemainingBGSubInvMassLeftPtBin, fFitRemainingBGInvMassLeftPtBin,
            nameMesonSub, nameCanvasSub, namePadSub, fMesonMassPlotRange, fDate, fPrefix, fRow, fColumn, fStartPtBin, fNBinsPt, fBinsPt, fTextMeasurement, fIsMC,
            fDecayChannel, fDetectionProcess, fCollisionSystem );
...
void Initialize(TString setPi0, Int_t numberOfBins, Int_t triggerSet){
    ...
    TString rotationBack = (isRotationBG == 0) ? "" : "rotation";
    InitializeWindows(setPi0, fMode, trigger, triggerSet, rotationBack);
...
void SetCorrectMCHistogrammNames(TString mesonType){
    ...
    // Pi0
        ...
        if(fMode == 5 && (fEnergyFlag.Contains("5TeV2017") || fEnergyFlag.Contains("PbPb_5.02TeV") || fEnergyFlag.Contains("13TeV"))) ObjectNameMCPi0AccWOWeights = "MC_Pi0InAcc_Pt";
        if(fMode == 4 || fMode == 12 || fMode == 5 || fMode==14)
...
void CreatePtHistos(){
    ...
    //--------------------------------------------------------
    // Histos for template fits
    //--------------------------------------------------------
    fHistoScaleFacBack                  = new TH1D("fHistoScaleFacBack","",fNBinsPt,fBinsPt);
    fHistoScaleFacBack->Sumw2();
    fHistoScaleFacPi0                   = new TH1D("fHistoScaleFacPi0","",fNBinsPt,fBinsPt);
    fHistoScaleFacPi0->Sumw2();
    fHistoScaleFacEta                   = new TH1D("fHistoScaleFacEta","",fNBinsPt,fBinsPt);
    fHistoScaleFacEta->Sumw2();

    fHistoScaleFacLeftBack                  = new TH1D("fHistoScaleFacLeftBack","",fNBinsPt,fBinsPt);
    fHistoScaleFacLeftBack->Sumw2();
    fHistoScaleFacLeftPi0                   = new TH1D("fHistoScaleFacLeftPi0","",fNBinsPt,fBinsPt);
    fHistoScaleFacLeftPi0->Sumw2();
    fHistoScaleFacLeftEta                   = new TH1D("fHistoScaleFacLeftEta","",fNBinsPt,fBinsPt);
    fHistoScaleFacLeftEta->Sumw2();
}
...
void FillPtHistos(){
    for(Int_t iPt=fStartPtBin+1;iPt<fNBinsPt+1;iPt++){
        ...
        if (fIsMC) {
            ...
            if (fFitSignalInvMassPtBinStd[iPt-1]!=0x00) {
                if(fFitSignalInvMassPtBinStd[iPt-1]->GetNpar() == 7){
...
void FitSubtractedInvMassInPtBins(TH1D* histoMappingSignalInvMassPtBinSingle, Double_t* mesonIntDeltaRangeFit, Int_t ptBin, Bool_t vary){
    ...
    if( fMode == 4 || fMode == 5  || fMode==14 || (fMode == 0 && fEnergyFlag.Contains("PbPb")) ){
        mesonAmplitude = 0;
    ...
    // Settings specific for pPb collisions
        ...
        if (fPrefix.Contains("Pi0")){
            if ( fMode == 0) {                                      // PCM
            ...
            } else if (fMode == 4 || fMode == 12 || fMode==14) {   // EMC, DMC
                if( fEnergyFlag.Contains("pPb_8TeV") ){
                    ...
                    TString trigger = fEventCutSelection(GetEventSelectSpecialTriggerCutPosition(),2);
                    if( (trigger.CompareTo("83") == 0 || trigger.CompareTo("85") == 0) && fBinsPt[ptBin] > 6.){
                      fMesonFitRange[0] = 0.08;
                      fMesonFitRange[1] = 0.30;
                      mesonAmplitudeMin = mesonAmplitude*90./100.;
                    }
    ...
    if( (fPrefix.Contains("Pi0")) && fEnergyFlag.BeginsWith("5TeV") && (trigger.CompareTo("a1") == 0 || trigger.CompareTo("a2") == 0)){
      fFitGausExp->SetParameter(4,fFitReco->GetParameter(6));
      fFitGausExp->SetParError(4,fFitReco->GetParError(6));
      ...
      fFitGausExp->SetParameter(4,fFitReco->GetParameter(6));
      fFitGausExp->SetParError(4,fFitReco->GetParError(6));
    } else if( (fPrefix.CompareTo("Pi0") ==0 || fPrefix.CompareTo("Pi0EtaBinning")==0) && fEnergyFlag.Contains("pPb_5.023TeV") && (trigger.CompareTo("83") || trigger.CompareTo("85"))  && fMode == 4 ){
      fFitGausExp->SetParameter(4,fFitReco->GetParameter(6));
      fFitGausExp->SetParError(4,fFitReco->GetParError(6));
    }
...
//****************************************************************************
//************** Fit of Signal+ BG with Gaussian + Exponential  **************
//****************************************************************************
void FitSubtractedInvMassInPtBinsWOLinear(TH1D* histoMappingSignalInvMassPtBinSingle, Double_t* mesonIntDeltaRangeFit, Int_t ptBin, Bool_t vary){
  //--------------------------------------------------------------------------------------
  // determine fit start values for amplitude and special settings for different collision
  // systems and energies
  //--------------------------------------------------------------------------------------
  ...
  if(fPrefix.Contains("Pi0")) fMesonMassExpect = 0.135;
  else if(fPrefix.Contains("Eta")) fMesonMassExpect = 0.548;
...
//   if ( fEnergyFlag.Contains("13TeV")  ){
    if(fPrefix.Contains("Pi0")){
      ...
      fFitReco->SetParLimits(1,fMesonMassExpect*0.92,fMesonMassExpect*1.1);
      fFitReco->SetParLimits(2,fMesonWidthRange[0],fMesonWidthRange[1]);
...
void FitTrueInvMassInPtBins(TH1D* histoMappingSignalInvMassPtBinSingle, Double_t* mesonIntDeltaRangeFit, Int_t ptBin, Bool_t vary)
{
    ...
    if (fEnergyFlag.CompareTo("PbPb_2.76TeV") == 0 || fEnergyFlag.Contains("XeXe")){
        ...
        if (fMode == 2 || fMode == 13 || fMode == 3){
            ...
            if( fEnergyFlag.BeginsWith("8TeV")  ){
                ...
                if (mesonBGString.CompareTo("r") == 0 && fPrefix.Contains("Pi0")){
                    doNoLinearFit = kTRUE;
                    mesonAmplitudeMin = mesonAmplitude;
                    mesonAmplitudeMax = mesonAmplitude*2000./100.;
                }
                if (fBinsPt[ptBin] > 10 && fPrefix.Contains("Pi0"))
                    doNoLinearFit = kTRUE;
    ...
    if (doDoubleExp){
        fFitReco = new TF1("fGaussExp","(x<[1])*([0]*(TMath::Exp(-0.5*((x-[1])/[2])^2)+TMath::Exp((x-[1])/[3])*(1.-TMath::Exp(-0.5*((x-[1])/[2])^2))))+(x>=[1])*([0]*(TMath::Exp(-0.5*((x-[1])/[2])^2)+TMath::Exp(-(x-[1])/[4])*(1.-TMath::Exp(-0.5*((x-[1])/[2])^2))))",fMesonFitRange[0],fMesonFitRange[1]);
        fFitReco->SetParameter(4,0.015);
        fFitReco->SetParLimits(4,0.010,0.030);
    } else if (fMode == 2 || fMode == 13 || fMode == 4 || fMode == 12 || fMode == 5 || fMode == 3){
    ...
    if ( (fMode == 2 || fMode == 4 || fMode == 12 || fMode == 13 || fMode == 5 || fMode == 3  || fMode==14) && (fEnergyFlag.Contains("pPb_5.023TeV") || fEnergyFlag.Contains("pPb_8TeV") )  )
...
void MakeBGFitSubtractedInvMassInPtBins(TH1D* histoMappingSignalInvMassPtBinSingle,Double_t * mesonIntDeltaRangeFit, Int_t ptBin,Bool_t vary,TString functionname ,Bool_t kMC){
    ...
    Bool_t bgNonZero     = kFALSE;
    for (Int_t i = 0; i< fFitLinearBck->GetNpar(); i++){
    ...
    if (bgNonZero){
        ...
        for (Int_t i = 1; i < fCopyOnlyBG->GetNbinsX()+1; i++){
            fCopyOnlyBG->SetBinContent(i, 0);
            fCopyOnlyBG->SetBinError(i, 0);
        }
        fIntLinearBck       = 0;
        fIntLinearBckError  = 0;
    }
...
//         graphClusterPtReb->Print();
    ...
    if(fMesonFullPtSignal)fMesonFullPtSignal->Write();
    if(fMesonFullPtBackground)fMesonFullPtBackground->Write();

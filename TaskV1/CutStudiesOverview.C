...
void CutStudiesOverview(TString CombineCutsName                 = "CombineCuts.dat",
    ...
    TH1D*   histoCorrectedYieldCutRebinned      [ConstNumberOfCuts]; // Copy of histoCorrectedYieldCut but rebinned if Option was used
    TH1D*   histoCorrectedYieldCutOriginalBinning = NULL; // copy of histoCorrectedYieldCut of first cut to save original Binning
    ...
    TH1D*   histoEtaToPi0CutSmoothed            [ConstNumberOfCuts]; // copy of histoEtaToPi0Cut potentially smoothed if option is enabled
    ...
    TH1D*   histoRatioCorrectedYieldCutRebinnedAndSmoothed [ConstNumberOfCuts]; // copy of histoRatioCorrectedYieldCut potentially rebinned and smoothed if those options are enabled
    TH1D*   histoRatioCorrectedYieldCutRebinned [ConstNumberOfCuts]; // copy of histoRatioCorrectedYieldCut saved with only rebinning done even though smoothing was enabled
...
//             }
            ...
            if( i == 0){ // save copy of histoCorrectedYieldCut of first cut to have original Binning
                histoCorrectedYieldCutOriginalBinning                           = (TH1D*)Cutcorrfile[i]->Get(nameCorrectedYield.Data());
                histoCorrectedYieldCutOriginalBinning->SetName(Form("%s_%s", nameCorrectedYield.Data(),cutNumber[i].Data()));
            }
    ...
    // Canvas to draw systErr of yield extraction pos and neg seperatly
...
//         }
        ...
        Double_t paramTCM[5]; // Paramters for TCM fit for smoothing
        ...
        if (histoCorrectedYieldCutRebinned[0]->GetXaxis()->GetNbins()<11) { // Low number of Bins would be flattened for many iterations
            NSmoothingIterations = histoCorrectedYieldCutRebinned[0]->GetXaxis()->GetNbins()-1;
        }
    ...
    //         if (mode == 2 || mode == 3 ) padEtaToPi0->SetLogy(1);
            ...
            for(Int_t i = 0; i< NumberOfCuts; i++){
                if(i == 0){
                    ...
                    if(doRebin) {
                        DrawGammaSetMarker(histoEtaToPi0CutSmoothed[i], 24, 1., color[0], color[0]);
                        histoEtaToPi0CutSmoothed[i]->DrawCopy("e1,p,same");
...
// cout << "line " << __LINE__ << endl;
        ...
        Int_t NBinsPt = histoCorrectedYieldCutOriginalBinning->GetNbinsX();
        ...
        for (Int_t i = 1; i < NBinsPt +1; i++){
            BinsXCenter[i]                  = histoCorrectedYieldCutOriginalBinning->GetBinCenter(i);
            BinsXWidth[i]                   = histoCorrectedYieldCutOriginalBinning->GetBinWidth(i)/2.;
        }
        ...
        const Int_t NB = histoCorrectedYieldCutOriginalBinning->GetXaxis()->GetNbins();
                ...
                if(smoothing ) {
                    if(histoRatioCorrectedYieldCutRebinnedAndSmoothed[0] && histoRatioCorrectedYieldCut[i] && histoRatioCorrectedYieldCutRebinnedAndSmoothed[i]){
                        if(i!=0) {
                            ...
                            if(i<20){
                                DrawGammaSetMarker(histoRatioCorrectedYieldCutRebinned[i], 24+i, 0.8,color[i]+2,color[i]+2);
                            } else {
                                DrawGammaSetMarker(histoRatioCorrectedYieldCutRebinned[i], 24+i, 0.8,color[i-20]+2,color[i-20]+2);
                            }
                            histoRatioCorrectedYieldCutRebinned[i]->DrawCopy("same,p,e1");
                            ...
                            if (doRebin)
                                legendmonitor4[i]->AddEntry(histoRatioCorrectedYieldCutRebinned[i], Form("%s rebinned",cutStringsName[i].Data()),"pl");
                            else
                                legendmonitor4[i]->AddEntry(histoRatioCorrectedYieldCutRebinned[i], Form("%s ",cutStringsName[i].Data()),"pl");
                            legendmonitor4[i]->AddEntry(histoRatioCorrectedYieldCutRebinnedAndSmoothed[i], "smoothed");//Form("%s smoothed",cutStringsName[i].Data()),"pl");

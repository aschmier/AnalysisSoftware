...
void CompareMesonQuantitiesOmega(    const char *dataFilename        = "rawSignalData",
                            ...
                            )
{
    ...
    TString decayChannel        = Form("%s #rightarrow #pi^{+}#pi^{-}#pi^{0}", textProcess.Data());
    ...
    TH1D* histoChi2_Data                        = (TH1D*) fileRawSignalData->Get("histoChi2_0");
    TH1D* histoChi2_SubPiZero_Data             = (TH1D*) fileRawSignalData->Get("histoChi2_1");
    TH1D* histoChi2_FixedPzPiZero_Data         = (TH1D*) fileRawSignalData->Get("histoChi2_2");
    TH1D* histoChi2_BackFit_Data               = (TH1D*) fileRawSignalData->Get("histoChi2_3");
    TH1D* histoChi2_BackFit_SubPiZero_Data     = (TH1D*) fileRawSignalData->Get("histoChi2_4");
    TH1D* histoChi2_BackFit_FixedPzPiZero_Data = (TH1D*) fileRawSignalData->Get("histoChi2_5");
    TH1D* histoChi2_MC                       = (TH1D*) fileRawSignalMC->Get("histoChi2_0");
    TH1D* histoChi2_SubPiZero_MC             = (TH1D*) fileRawSignalMC->Get("histoChi2_1");
    TH1D* histoChi2_FixedPzPiZero_MC         = (TH1D*) fileRawSignalMC->Get("histoChi2_2");
    TH1D* histoChi2_BackFit_MC               = (TH1D*) fileRawSignalMC->Get("histoChi2_3");
    TH1D* histoChi2_BackFit_SubPiZero_MC     = (TH1D*) fileRawSignalMC->Get("histoChi2_4");
    TH1D* histoChi2_BackFit_FixedPzPiZero_MC = (TH1D*) fileRawSignalMC->Get("histoChi2_5");
    TH1D* histoRatioYieldLowMassDataDivMC   = (TH1D*) histoChi2_BackFit_SubPiZero_Data->Clone("histoRatioYieldLowMassDataDivMC");

    Double_t fMesonRange[2] = {0., 0.79};
    TString outputDir                       = Form("%s/%s/%s/ExtractSignal",fCutSelection.Data(),energyFlag.Data(),fSuffix.Data());
    TString nameLineShapePlot               = Form("%s/%s_MesonLineShapeComparedBackFit_%s.%s",outputDir.Data(),mesonType.Data(),fCutSelection.Data(),fSuffix.Data());
    TString nameLineShapePlotLeft           = Form("%s/%s_MesonLineShapeComparedBackFit_SubPiZero_%s.%s",outputDir.Data(),mesonType.Data(),fCutSelection.Data(),fSuffix.Data());
    TString nameLineShapePlotFull           = Form("%s/%s_MesonLineShapeComparedBackFit_WithChi2_SubPiZero_%s.%s",outputDir.Data(),mesonType.Data(),fCutSelection.Data(),fSuffix.Data());

    TString cutSSelection              = "";
    ...
    if((mode>=40) && (mode <= 50)) isNewTask = kFALSE;
       fMode                       = ReturnSeparatedCutNumberPiPlPiMiPiZero( fCutSelection, fTypeCutSelection, fEventCutSelection, fGammaCutSelection, fClusterCutSelection,
    ...
    for(Int_t j=0;j<3;j++){
      cout << "j = " << j << " fNBinsPt = " << fNBinsPt << endl;
        ...
        for(Int_t iPt=fStartPtBin; iPt<fNBinsPt; iPt++){
            ...
            if(j==0){
                histonameSignal = Form("fHistoMappingSignalBackFitInvMass_in_Pt_Bin%02d", iPt);
            } else if(j==1) {
                histonameSignal = Form("fHistoMappingSignalBackFitInvMass_SubPiZero_in_Pt_Bin%02d", iPt);
            } else if(j==2) {
                histonameSignal = Form("fHistoMappingSignalBackFitInvMass_SubPiZero_in_Pt_Bin%02d", iPt);
            }
...
/*
            ...
            if (j == 0 ){
                ...
                histoTrueMCInvMassPtBin[iPt]->Scale(1./integralMC);
            } 
            if (j > 0){
                histonameMCTruth = Form("Mapping_TrueMeson_InvMass_in_Pt_Bin%02d_SubNDM", iPt);
                histoTrueMCInvMassPtBin[iPt]=(TH1D*)fileRawSignalMC->Get(histonameMCTruth);
                histoTrueMCInvMassPtBin[iPt]->Scale(1./integralMC);
            }
            if (j == 1){
    ...
    DrawGammaCanvasSettings( canvasChi2, 0.092, 0.01, 0.02, 0.082);
        ...
        Double_t maxChi2    = histoChi2_Data->GetMaximum();
        if (maxChi2 < histoChi2_MC->GetMaximum())
            maxChi2         = histoChi2_MC->GetMaximum();
        maxChi2             = maxChi2*1.2;

        histoChi2_Data->GetYaxis()->SetRangeUser(0, maxChi2);
        DrawAutoGammaMesonHistos( histoChi2_Data,
                                    "", "#it{p}_{T} (GeV/#it{c})", "#it{#chi}^{2}/ndf",
                                    kFALSE, 0., 0.7, kFALSE,
                                    kFALSE, 0., 0.7,
                                    kFALSE, 0., 10.);
        DrawGammaSetMarker(histoChi2_Data, 20, 2, kBlack, kBlack);
        histoChi2_Data->DrawCopy("same,e1,p");
        DrawGammaSetMarker(histoChi2_MC, 24, 2, kRed+1, kRed+1);
        histoChi2_MC->DrawCopy("same,e1,p");

        TLegend* legendChi2 = GetAndSetLegend2(0.85, 0.13, 0.95, 0.13+(0.035*2), 0.035, 1, "", 42, 0.25);
        legendChi2->AddEntry(histoChi2_Data,"Data");
        legendChi2->AddEntry(histoChi2_MC,"MC");
        legendChi2->Draw();
    ...
    canvasChi2->cd();
        maxChi2    = histoChi2_SubPiZero_Data->GetMaximum();
        if (maxChi2 < histoChi2_SubPiZero_MC->GetMaximum())
            maxChi2         = histoChi2_SubPiZero_MC->GetMaximum();
        maxChi2             = maxChi2*1.2;

        histoChi2_SubPiZero_Data->GetYaxis()->SetRangeUser(0, maxChi2);
        DrawAutoGammaMesonHistos( histoChi2_SubPiZero_Data,
                                    "", "#it{p}_{T} (GeV/#it{c})", "#it{#chi}^{2}/ndf",
                                    kFALSE, 0., 0.7, kFALSE,
                                    kFALSE, 0., 0.7,
                                    kFALSE, 0., 10.);
        DrawGammaSetMarker(histoChi2_SubPiZero_Data, 20, 2, kBlack, kBlack);
        histoChi2_SubPiZero_Data->DrawCopy("same,e1,p");
        DrawGammaSetMarker(histoChi2_SubPiZero_MC, 24, 2, kRed+1, kRed+1);
        histoChi2_SubPiZero_MC->DrawCopy("same,e1,p");

        legendChi2->Draw();
        PutProcessLabelAndEnergyOnPlot(0.15, 0.25, 0.035, energyText.Data(), decayChannel.Data(), DetectionChannel.Data());

    canvasChi2->Update();
    canvasChi2->SaveAs(Form("%s/%s_Chi2_SubPiZero_%s.%s",outputDir.Data(),mesonType.Data(),fCutSelection.Data(),fSuffix.Data()));

    canvasChi2->cd();
        maxChi2    = histoChi2_FixedPzPiZero_Data->GetMaximum();
        if (maxChi2 < histoChi2_FixedPzPiZero_MC->GetMaximum())
            maxChi2         = histoChi2_FixedPzPiZero_MC->GetMaximum();
        maxChi2             = maxChi2*1.2;

        histoChi2_FixedPzPiZero_Data->GetYaxis()->SetRangeUser(0, maxChi2);
        DrawAutoGammaMesonHistos( histoChi2_FixedPzPiZero_Data,
                                    "", "#it{p}_{T} (GeV/#it{c})", "#it{#chi}^{2}/ndf",
                                    kFALSE, 0., 0.7, kFALSE,
                                    kFALSE, 0., 0.7,
                                    kFALSE, 0., 10.);
        DrawGammaSetMarker(histoChi2_FixedPzPiZero_Data, 20, 2, kBlack, kBlack);
        histoChi2_FixedPzPiZero_Data->DrawCopy("same,e1,p");
        DrawGammaSetMarker(histoChi2_FixedPzPiZero_MC, 24, 2, kRed+1, kRed+1);
        histoChi2_FixedPzPiZero_MC->DrawCopy("same,e1,p");

        legendChi2->Draw();
        PutProcessLabelAndEnergyOnPlot(0.15, 0.25, 0.035, energyText.Data(), decayChannel.Data(), DetectionChannel.Data());

    canvasChi2->Update();
    canvasChi2->SaveAs(Form("%s/%s_Chi2_FixedPzPiZero_%s.%s",outputDir.Data(),mesonType.Data(),fCutSelection.Data(),fSuffix.Data()));

    // With Back fit
     canvasChi2->cd();
     maxChi2    = histoChi2_BackFit_Data->GetMaximum();
        if (maxChi2 < histoChi2_BackFit_MC->GetMaximum())
            maxChi2         = histoChi2_BackFit_MC->GetMaximum();
        maxChi2             = maxChi2*1.2;

        histoChi2_BackFit_Data->GetYaxis()->SetRangeUser(0, maxChi2);
        DrawAutoGammaMesonHistos( histoChi2_BackFit_Data,
                                    "", "#it{p}_{T} (GeV/#it{c})", "#it{#chi}^{2}/ndf",
                                    kFALSE, 0., 0.7, kFALSE,
                                    kFALSE, 0., 0.7,
                                    kFALSE, 0., 10.);
        DrawGammaSetMarker(histoChi2_BackFit_Data, 20, 2, kBlack, kBlack);
        histoChi2_BackFit_Data->DrawCopy("same,e1,p");
        DrawGammaSetMarker(histoChi2_BackFit_MC, 24, 2, kRed+1, kRed+1);
        histoChi2_BackFit_MC->DrawCopy("same,e1,p");

        legendChi2 = GetAndSetLegend2(0.85, 0.13, 0.95, 0.13+(0.035*2), 0.035, 1, "", 42, 0.25);
        legendChi2->AddEntry(histoChi2_BackFit_Data,"Data");
        legendChi2->AddEntry(histoChi2_BackFit_MC,"MC");
        legendChi2->Draw();
        PutProcessLabelAndEnergyOnPlot(0.15, 0.25, 0.035, energyText.Data(), decayChannel.Data(), DetectionChannel.Data());

    canvasChi2->Update();
    canvasChi2->SaveAs(Form("%s/%s_Chi2_BackFit_%s.%s",outputDir.Data(),mesonType.Data(),fCutSelection.Data(),fSuffix.Data()));

    canvasChi2->cd();
        maxChi2    = histoChi2_BackFit_SubPiZero_Data->GetMaximum();
        if (maxChi2 < histoChi2_BackFit_SubPiZero_MC->GetMaximum())
            maxChi2         = histoChi2_BackFit_SubPiZero_MC->GetMaximum();
        maxChi2             = maxChi2*1.2;

        histoChi2_BackFit_SubPiZero_Data->GetYaxis()->SetRangeUser(0, maxChi2);
        DrawAutoGammaMesonHistos( histoChi2_BackFit_SubPiZero_Data,
                                    "", "#it{p}_{T} (GeV/#it{c})", "#it{#chi}^{2}/ndf",
                                    kFALSE, 0., 0.7, kFALSE,
                                    kFALSE, 0., 0.7,
                                    kFALSE, 0., 10.);
        DrawGammaSetMarker(histoChi2_BackFit_SubPiZero_Data, 20, 2, kBlack, kBlack);
        histoChi2_BackFit_SubPiZero_Data->DrawCopy("same,e1,p");
        DrawGammaSetMarker(histoChi2_BackFit_SubPiZero_MC, 24, 2, kRed+1, kRed+1);
        histoChi2_BackFit_SubPiZero_MC->DrawCopy("same,e1,p");

        legendChi2->Draw();
        PutProcessLabelAndEnergyOnPlot(0.15, 0.25, 0.035, energyText.Data(), decayChannel.Data(), DetectionChannel.Data());

    canvasChi2->Update();
    canvasChi2->SaveAs(Form("%s/%s_Chi2_BackFit_SubPiZero_%s.%s",outputDir.Data(),mesonType.Data(),fCutSelection.Data(),fSuffix.Data()));

    canvasChi2->cd();
        maxChi2    = histoChi2_BackFit_FixedPzPiZero_Data->GetMaximum();
        if (maxChi2 < histoChi2_BackFit_FixedPzPiZero_MC->GetMaximum())
            maxChi2         = histoChi2_BackFit_FixedPzPiZero_MC->GetMaximum();
        maxChi2             = maxChi2*1.2;

        histoChi2_BackFit_FixedPzPiZero_Data->GetYaxis()->SetRangeUser(0, maxChi2);
        DrawAutoGammaMesonHistos( histoChi2_BackFit_FixedPzPiZero_Data,
                                    "", "#it{p}_{T} (GeV/#it{c})", "#it{#chi}^{2}/ndf",
                                    kFALSE, 0., 0.7, kFALSE,
                                    kFALSE, 0., 0.7,
                                    kFALSE, 0., 10.);
        DrawGammaSetMarker(histoChi2_BackFit_FixedPzPiZero_Data, 20, 2, kBlack, kBlack);
        histoChi2_BackFit_FixedPzPiZero_Data->DrawCopy("same,e1,p");
        DrawGammaSetMarker(histoChi2_BackFit_FixedPzPiZero_MC, 24, 2, kRed+1, kRed+1);
        histoChi2_BackFit_FixedPzPiZero_MC->DrawCopy("same,e1,p");

        legendChi2->Draw();
        PutProcessLabelAndEnergyOnPlot(0.15, 0.25, 0.035, energyText.Data(), decayChannel.Data(), DetectionChannel.Data());

    canvasChi2->Update();
    canvasChi2->SaveAs(Form("%s/%s_Chi2_BackFit_FixedPzPiZero_%s.%s",outputDir.Data(),mesonType.Data(),fCutSelection.Data(),fSuffix.Data()));

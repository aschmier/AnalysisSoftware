...
void JetQA_Runwise_pPb(
        ...
        Int_t triggerNr                 = -1,               // 0 = EJ1, 1 = EJ2, 2 = INT7
        ...
        TString addLabelRunListMC       = "",               // additional name for MC runlist
        TString fixedTopDir             = ""
        )
{
    cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << endl;
    ...
    TString fTrigger;
    if(triggerNr == 0) fTrigger    = "EJ1";
    else if(triggerNr == 1) fTrigger    = "EJ2";
    ...
    for(Int_t i=0; i<nSets; i++){
        ...
        cout << "Energy Flag: " << fEnergyFlag.Data() << endl;
    ...
    for(Int_t i=0; i<nSets; i++){
        if(i >= nData) fileRuns[i]  = Form("%s/runNumbers%s_%s.txt", folderRunlists.Data(), (vecDataSet.at(i)).Data(), addLabelRunListMC.Data());
        else fileRuns[i]            = Form("%s/runNumbers%s_%s.txt", folderRunlists.Data(), (vecDataSet.at(i)).Data(), addLabelRunList.Data());
    ...
    while ( (fInputFile==NULL)&&(ActualRunIndexInVector<vecRuns.size()) ){
        fInputFile             = new TFile(Form("%s/%s/pass2_merged/%s/%s/%s", filePath.Data(), ((TString)vecDataSet.at(0)).Data(), addLabelRunList.Data(), ((TString)vecRuns.at(ActualRunIndexInVector)).Data(), fileName.Data()));
        cout << "Input file: " << fInputFile->GetName() << endl;
        if(fInputFile->IsZombie()) {
            cout << "ERROR: ROOT file '" << Form("%s/%s/pass2_merged/%s/%s/%s", filePath.Data(), ((TString)vecDataSet.at(0)).Data(), addLabelRunList.Data(), ((TString)vecRuns.at(ActualRunIndexInVector)).Data(), fileName.Data()) << "' could not be openend, return!" << endl;
            fInputFile->Close();
            delete fInputFile;
            fInputFile=NULL;
        }
    ...
    if(fInputFile==NULL) {
        cout << "ERROR: no ROOT file; last tried file: '" << Form("%s/%s/pass2_merged/%s/%s/%s", filePath.Data(), ((TString)vecDataSet.at(0)).Data(), addLabelRunList.Data(), ((TString)vecRuns.at(ActualRunIndexInVector-1)).Data(), fileName.Data()) << "'; return!" << endl;
        return;
    }
    ...
    TString fTempTriggerSel              = fTriggerSelection;
    TString fSaveTriggerSelection        = (fTempTriggerSel.ReplaceAll("_nodownscalecorr","").ReplaceAll("_default",""));
    TString fEventTriggerSelection       = "";

    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    //++++++++++++++++++++++++++ Define output directories+++++++++++++++++++++++++++++++++++++++++++++++++
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    TString outputDir           = Form("%s_%s/%s/JetQA/%s/Runwise",fSaveTriggerSelection.Data(),addLabelRunList.Data(),fEnergyFlag.Data(),suffix.Data());
    if (fEnergyFlag.Contains("13TeV")) {
        outputDir       = Form("%s_%s/%s/JetQA/%s/%s/Runwise", fSaveTriggerSelection.Data(),addLabelRunList.Data(),fEnergyFlag.Data(),suffix.Data(), DataSets[0].Data());
    }
    if(nDataIn == 1){
    ...
    for(Int_t i=0; i<nSets; i++) {
        ...
        for(Int_t j=0; j<(Int_t) vecRuns.size(); j++){
            ...
            // MC reader
            if(i >= nData) fRootFile    = Form("%s/%s_%s/runwise/%s/%s", filePathMC.Data(), fDataSet.Data(), mcFolderExtension.Data(), fRunNumber.Data(), fileName.Data());
            // Data reader
            else fRootFile              = Form("%s/%s/pass2_merged/%s/%s/%s", filePath.Data(), fDataSet.Data(), addLabelRunList.Data(), fRunNumber.Data(), fileName.Data());
            ...
            if(!TopDir){
                if(fTriggerSelection.Contains("nodownscalecorr")) fTriggerSelection.ReplaceAll("nodownscalecorr","default");
                else if(fTriggerSelection.Contains("default")) fTriggerSelection.ReplaceAll("default","nodownscalecorr");
                else{ cout << "Directory name doesn't contain nodownscalecorr or default!" << endl; return; }
                TopDir                      = (TDirectory*)RootFile->Get(fTriggerSelection.Data());
            }
            if(TopDir == NULL) {cout << "ERROR: TopDir not Found: " << fTriggerSelection.Data() << endl; return;}
            ...
            if(JetPt2D){
                ...
                else if(fTrigger == "EJ2" || fTrigger == "EJ1"){
                  tempNXbins = 33;
            ...
            if(JetPtMax2D){
                ...
                if(fTrigger == "EJ2") JetPtMax->GetXaxis()->SetRangeUser(5,350);
                if(fTrigger == "EJ1") JetPtMax->GetXaxis()->SetRangeUser(16,350);
            ...
            if(NEFrac2D){
                if(i < nData && nEvents > 0){
                    ...
                    for (Int_t binPt = 0; binPt < nPtBins; binPt++){
                        ...
                        if(fTrigger == "EJ2" && (binsPt[binPt] < 20 || binsPt[binPt+1] > 100)) continue;
                        if(fTrigger == "EJ1"  && (binsPt[binPt] < 30 || binsPt[binPt+1] > 350)) continue;
...
/*
    ...
    if(doHistsForEverySet) {
        ...
        //TString* ratioSets2[nSets-nData];
...
/*
            for(Int_t h=0; h<(Int_t) vecHistosTH1F[0].size(); h++) {
                for(Int_t i=0; i<nData; i++) {
                    ...
                    for(Int_t j=0; j<nSets-nData; j++){
                        ratioSets2[j] = new TString(Form("%s / %s", plotDataSets[i].Data(), plotDataSets[j+nData].Data()));
                    ...
                    for(Int_t j=0; j<nSets-nData; j++) {
                        ...
                        legend->AddEntry(ratio2[j],ratioSets2[j]->Data(),"p");
                    }
                    if (drawVerticalLines){
                        canvas->Update();
                        yMax = canvas->GetUymax();
...
*/
    ...
    for(Int_t i=0; i<nSets; i++){
        ...
        if(useDataRunListForMC && i>=nData) nameOutput = Form("%s_%s/%s/JetQA/%s-%s_JetQARunwise.root",fSaveTriggerSelection.Data(),addLabelRunList.Data(),fEnergyFlag.Data(),fDataSet.Data(),vecDataSet.at(0).Data());
        else nameOutput = Form("%s_%s/%s/JetQA/%s_JetQARunwise.root",fSaveTriggerSelection.Data(),addLabelRunList.Data(),fEnergyFlag.Data(),fDataSet.Data());

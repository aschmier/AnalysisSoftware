...
void Grid_CopyFiles_Runwise_MCAOD_anch18_AODVal(TString folder = "/home/jens/Programme_und_Skripte/alice-pcg/AnalysisSoftware", Int_t TrainNumberChoiceArgument=1, Int_t doUnzipArgument=1, Bool_t isJetJet=kFALSE)
{
    ...
    TString DataSets[nSets]={
        Form("FILTER_p-p_%s_LHC18g4",  FilterString.Data()),
        Form("FILTER_p-p_%s_LHC18g5",  FilterString.Data()),
        Form("FILTER_p-p_%s_LHC18g6",  FilterString.Data()),
        Form("FILTER_p-p_%s_LHC18h2",  FilterString.Data()),
        Form("FILTER_p-p_%s_LHC18h4",  FilterString.Data()),
        Form("FILTER_p-p_%s_LHC18j1",  FilterString.Data()),
        Form("FILTER_p-p_%s_LHC18j4",  FilterString.Data()),
        Form("FILTER_p-p_%s_LHC18k1",  FilterString.Data()),
        Form("FILTER_p-p_%s_LHC18k2",  FilterString.Data()),
        Form("FILTER_p-p_%s_LHC18k3",  FilterString.Data())
    };
    ...
    for(Int_t i=nSetsStart; i<nSetsEnd; i++)
    {
        ...
        for(Int_t j=0; j<(Int_t)vecRuns.size(); j++)
        {
            ...
            //if((vecRuns.at(j).CompareTo("287353")==0)||(vecRuns.at(j).CompareTo("289176")==0)||(vecRuns.at(j).CompareTo("289935")==0)) continue;
